...
<body class="text-gray-800 bg-gray-100">
    ...
    <section class="relative flex flex-col mx-4 mb-6 space-y-6 md:flex-row md:space-y-0 md:space-x-6">
                        ...
                        <div id="human-dropdown" class="hidden mt-2 ml-4 space-y-2">
                            <label class="block">
                                <input 
                                    ...
                                    value="human" 
                                    class="mr-2" 
                                    {{ request('filter') && in_array('human', request('filter')) ? 'checked' : '' }}>
                                Human Food
                            </label>
                        </div>
                    </div>
                        ...
                        <div id="animals-dropdown" class="hidden mt-2 ml-4 space-y-2">
                            <label class="block">
                                <input 
                                    ...
                                    value="animal" 
                                    class="mr-2" 
                                    {{ request('filter') && in_array('animal', request('filter')) ? 'checked' : '' }}>
                                Animal Feed
                            </label>
                        </div>
                    </div>
                        ...
                        <div id="fertilizer-dropdown" class="hidden mt-2 ml-4 space-y-2">
                            <label class="block">
                                <input 
                                    ...
                                    value="fertilizer" 
                                    class="mr-2" 
                                    {{ request('filter') && in_array('fertilizer', request('filter')) ? 'checked' : '' }}>
                                Fertilizer
                            </label>
                        </div>
                    </div>
        ...
        <div class="grid grid-cols-2 gap-8 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5">
            ...
            <a href="{{ route('shop.product.show', $product->id) }}" class="relative transition-transform bg-white border border-gray-200 rounded-lg shadow-lg hover:shadow-xl hover:scale-105">
                <!-- Flash Sale Badge -->
                ...
                <div class="p-3">
                    ...
                    <p class="mt-2 font-bold text-green-600">
                        Rs.{{ $product->is_flash_sale ? $product->flash_sale_price : $product->price }}
                        @if ($product->is_flash_sale)
                            <span class="ml-2 line-through text-red-500">Rs.{{ $product->price }}</span>
                        @endif
                    </p>
                </div>
            </a>
