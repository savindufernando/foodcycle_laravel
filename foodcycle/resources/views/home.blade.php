...
<body>
    ...
    <!-- Hero Section -->
    <!-- Hero Section -->
<section class="relative h-screen bg-fixed bg-center bg-cover" style="background-image: url('{{ asset('panel/image/banner.jpg') }}');">
    <!-- Background Overlay -->
                ...
                @foreach ($flashSales as $product)
                    <div class="relative min-w-[250px] bg-white border border-gray-200 rounded-lg shadow-md overflow-hidden">
                        ...
                        <div class="p-4">
                            ...
                            <p class="mt-2 text-xl font-bold text-green-600">Rs. {{ number_format($product->flash_sale_price, 2) }}</p>
                            <p class="text-sm line-through text-gray-400">Rs. {{ number_format($product->price, 2) }}</p>
                            <p class="mt-1 text-sm text-gray-700">Seller: {{ $product->admin->name }}</p>

                            <!-- Buy Now Button -->
                            <div class="mt-4 text-center">
                                <a href="{{ route('shop.product.show', $product->id) }}" class="px-4 py-2 text-white bg-blue-500 rounded-md hover:bg-blue-600">
                                    Buy Now
                                </a>
                            </div>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        @foreach ($flashSales as $product)
            let countdown{{ $product->id }} = {{ $product->remaining_hours * 3600 + $product->remaining_minutes * 60 }}; // Convert to seconds
