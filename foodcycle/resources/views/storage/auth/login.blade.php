...
<head>
    ...
    <title>Storage Provider Login</title>
  ...
  <section class="flex items-center justify-center min-h-screen pt-16 bg-gray-100 bg-center bg-cover">
    ...
    <div class="relative z-10 flex flex-col w-full max-w-4xl overflow-hidden bg-white rounded-lg shadow-lg md:flex-row">
      ...
      <div class="w-full p-8 md:w-1/2">
        <h2 class="mb-4 text-3xl font-bold">Storage Provider Login</h2>
  
        <x-auth-session-status class="mb-4" :status="session('status')" />
  
        <!-- Form -->
        <form action="{{ route('storage.login') }}" method="POST">
          @csrf
          ...
          <p class="mb-6 text-gray-600">Don't have an account? <a class="text-blue-500" href="{{ route('storage.signup') }}">
