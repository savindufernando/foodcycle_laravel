...
<body class="text-gray-800 bg-gray-100">
    ...
    <main class="max-w-6xl p-8 mx-auto my-10 bg-white rounded-lg shadow-lg">
        <!-- Stylish Back Button -->
        <div class="mb-6">
            <a href="{{ route('shop.index') }}" class="inline-flex items-center px-4 py-2 text-white transition duration-300 ease-in-out transform bg-blue-600 rounded-full hover:bg-blue-700 hover:scale-105">
        ...
        <div class="md:flex md:space-x-8">
            ...
            <div class="flex justify-center md:w-1/2">
                <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="object-cover rounded-lg shadow-lg h-80">
            </div>
            ...
            <div class="md:w-1/2">
                ...
                <div class="mt-2">
                    <span class="text-2xl font-bold text-green-600">
                        Rs.{{ $product->is_flash_sale ? number_format($product->flash_sale_price, 2) : number_format($product->price, 2) }}
                    </span>
                    @if ($product->is_flash_sale)
                        <span class="ml-2 text-red-500 line-through">Rs.{{ number_format($product->price, 2) }}</span>
                    @endif
                </div>

                <p class="mt-4 text-gray-600">{{ $product->description }}</p>
                <p class="mt-4 text-gray-500">Category: <span class="font-medium text-gray-800">{{ $product->category }}</span></p>
