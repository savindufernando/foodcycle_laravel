...
<body class="bg-gray-100">
    ...
    <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
        <ul class="space-y-2 font-medium">
            <li>
                <a href="{{ route('blogger.dashboard') }}" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div id="blogContainer" class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                ...
                <div class="bg-white shadow-lg rounded-lg overflow-hidden blog-card approved">
                    ...
                    <div class="p-4">
                        ...
                        <div class="mt-4 flex justify-between items-center">
                            <a href="{{ route('blogs.show', $blog->id) }}" class="text-green-500 hover:underline">View</a>
            ...
            @foreach($pendingBlogs as $blog)
                <div class="bg-white shadow-lg rounded-lg overflow-hidden blog-card pending">
                    ...
                    <div class="p-4">
                        ...
                        <p class="text-sm text-gray-600 mt-2">{{ Str::limit(strip_tags($blog->content), 80) }}</p>
                        <div class="mt-4 flex justify-between items-center">
                            <a href="{{ route('blogs.show', $blog->id) }}" class="text-yellow-500 hover:underline">View</a>
                            <span class="text-xs text-gray-500">{{ $blog->created_at->format('M d, Y') }}</span>
                        </div>
                    </div>
                </div>
            ...
            <!-- Rejected Blogs -->
            @foreach($rejectedBlogs as $blog)
                <div class="bg-white shadow-lg rounded-lg overflow-hidden blog-card rejected">
                    <img src="{{ asset('storage/' . $blog->photos1) }}" alt="{{ $blog->title }}" class="w-full h-40 object-cover">
                    <div class="p-4">
                        <h3 class="text-lg font-semibold text-gray-800">{{ $blog->title }}</h3>
                        <p class="text-sm text-red-600 mt-2">Reason: {{ $blog->rejected_reason }}</p>
                        <p class="text-sm text-gray-600 mt-2">{{ Str::limit(strip_tags($blog->content), 80) }}</p>
                        <div class="mt-4 flex justify-between items-center">
                            <a href="{{ route('blogs.show', $blog->id) }}" class="text-red-500 hover:underline">View</a>
                            <span class="text-xs text-gray-500">{{ $blog->created_at->format('M d, Y') }}</span>
                        </div>
                    </div>
                </div>
